...
export function TaskCard({ task, onEdit, onDelete, onComplete }) {
  ...
  return (
    ...
    >
      <Card className={`p-3 sm:p-4 transition-all duration-200 hover:shadow-raised rounded-md ${
        task.is_completed ? 'opacity-60 bg-base-850' : 'bg-base-850'
      } ${isOverdue ? 'border-error-500 bg-error-900/40' : 'border-border'}`}>
        <div className="flex items-start justify-between">
          <div className="flex-1 space-y-2 sm:space-y-3 min-w-0">
            <div className="flex items-start justify-between">
              <h4 className={`font-semibold text-content-primary text-sm sm:text-base truncate pr-2 ${
                task.is_completed ? "line-through text-content-muted" : ""
              }`}>
                {task.name}
              </h4>
              {task.is_completed && (
                <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-success-500 flex-shrink-0" strokeWidth={1.5} />
              )}
            </div>
            <p className="text-xs sm:text-sm text-content-secondary font-medium truncate">{task.subject}</p>
            <div className="flex flex-wrap items-center gap-1 sm:gap-2">
              ...
              <div className="flex items-center text-content-muted text-xs">
                <Clock size={12} className="sm:w-3.5 sm:h-3.5 mr-1" strokeWidth={1.5}/>
                {task.estimated_time} min
              </div>
            </div>
            <div className={`text-xs sm:text-sm font-medium ${
              isOverdue ? 'text-error-400' : task.is_completed ? 'text-success-400' : 'text-content-secondary'
            }`}>
              {task.is_completed ? 'Completed' : taskDate ? `Due: ${formatDistanceToNow(taskDate, { addSuffix: true })}` : 'No deadline'}
            </div>
            {task.description && (
              <p className="text-xs text-content-muted line-clamp-2">{task.description}</p>
            )}
          </div>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-36 sm:w-40">
              <DropdownMenuItem onSelect={onComplete} className="flex items-center text-xs sm:text-sm">
                <CheckCircle size={12} className="sm:w-3.5 sm:h-3.5 mr-2" strokeWidth={1.5}/>
                {task.is_completed ? "Mark Incomplete" : "Mark Complete"}
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={onEdit} className="flex items-center text-xs sm:text-sm">
                <Edit size={12} className="sm:w-3.5 sm:h-3.5 mr-2" strokeWidth={1.5}/>Edit
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={onDelete} className="flex items-center text-error-400 hover:text-error-300 hover:bg-error-900/20 text-xs sm:text-sm">
                <Trash2 size={12} className="sm:w-3.5 sm:h-3.5 mr-2" strokeWidth={1.5}/>Delete
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
