  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        ...
        <div className="text-xs text-slate-400">{perf.description}</div>
      </div>
      <div className="text-right">
        <div className={`text-sm font-mono ${perf.color}`}>{responseTime.toFixed(2)}s</div>
        <div className="text-xs text-slate-500">{perf.emoji}</div>
      </div>
    </motion.div>
  );
};
      ...
      <Card className="p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-slate-400">Sub-second</div>
          </div>
        </div>
      </Card>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-medium text-slate-300">Speed Goal Progress</span>
        <Badge 
          variant={isAchieving ? "success" : "warning"}
          className={isAchieving ? "bg-green-900 text-green-200" : "bg-yellow-900 text-yellow-200"}
        >
      ...
      <div className="relative">
        <div className="w-full bg-slate-700 rounded-full h-2">
          <motion.div
            className={`h-2 rounded-full ${isAchieving ? 'bg-green-500' : 'bg-blue-500'}`}
            initial={{ width: 0 }}
      ...
      {isAchieving && (
        ...
        >
          <span className="text-green-400 text-sm">⚡ Sub-second responses achieved!</span>
        </motion.div>
      )}
    </motion.div>
  );
